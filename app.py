...
def send_message(
    ...
    try:
        ...
        if not response.ok:
            ...
            # If we were replying to a specific message and
            # Telegram says that message no longer exists (it
            # can be deleted/edited during a slow cold-start
            # reply), retry once WITHOUT the reply link instead
            # of losing the message entirely.
            if reply_to_message_id:
                ...
                if reply_target_gone:
                    ...
                    logger.warning(
                        "Reply target missing for chat_id=%s, "
                        "retrying without reply_to_message_id.",
                        chat_id,
                    )
                    ...
                    try:
                        ...
                        if not retry_response.ok:
                            logger.error(
                                "sendMessage retry (no reply) "
                                "also failed chat_id=%s: %s",
                                chat_id,
                                retry_response.text,
                            )
...
def ask_ai(user_text):
    """
    Reliable Gemini AI request with automatic retry/backoff.

    Handles:
    - 429 rate limits (exponential backoff, honors Retry-After)
    - temporary API errors (500/502/503/504, retried)
    - connection/request errors (retried)
    - malformed responses / missing candidates / blocked output

    Returns the AI's reply as a clean string, or None if every
    attempt fails. Callers should handle a None return (e.g. by
    showing their own fallback message to the user).
    """
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            if response.ok:
                ...
                if not candidates:
                    # Can happen when the prompt/response was
                    # blocked by safety filters (promptFeedback
                    # will usually explain why).
                    logger.error(
            ...
            if response.status_code == 429:
                ...
                logger.error(
                    "Gemini rate limit exhausted all retries."
                )
            ...
            if response.status_code in (500, 502, 503, 504):
                ...
                logger.error(
                    "Gemini server errors exhausted all retries."
                )
        ...
        # --------------------------------------------
            ...
            logger.exception(
                "Unexpected ask_ai error attempt=%s/%s",
                attempt + 1,
                max_attempts,
            )
...
def webhook():
    ...
    if bot_was_addressed(message):
        ...
        if reply is None:
            reply = (
                "Sorry, I'm having trouble answering right "
